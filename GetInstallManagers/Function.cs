...
namespace GetInstallManagers
{
    public class Function
    {
        ...
        public async Task<object> FunctionHandler(object input, ILambdaContext context)
        {
            ...
            var orgId = inp["path"].ToString().Split("/").Last();

            var installers = await GetInstallManagers.RetrieveInstallManagers(orgId).ConfigureAwait(true);

            var resp = JsonConvert.SerializeObject(installers);
